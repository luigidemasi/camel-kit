...
> **Prerequisite:** Phase 1 (`biztalk-phase1.md`) and the shared behavioral analysis must be complete:
...
**Conditionally load:**
- `skills/camel-migrate/guides/biztalk-map-conversion.md` — load once per orchestration that contains a BizTalk map
  (see Step 2.2)
...
Before designing orchestrations, map every `Inferred` or `Unknown` `MIG-###` row to an explicit design constraint,
validation requirement, or unresolved decision. Preserve the risk ID and status; Phase 2 must not silently declare it
resolved.
...
| **Transform Shape** | Canonical inline Groovy or XSLT DataMapper | `camel-groovy` or `camel-xslt-saxon` | Canonicalize first; see `biztalk-map-conversion.md`. |
...
- **BizTalk maps:** For each orchestration containing a Transform shape that uses a BizTalk map (`.btm` file), load
  `skills/camel-migrate/guides/biztalk-map-conversion.md`. Extract semantic mappings and schema paths from the BizTalk
  artifacts, confirm ambiguities, then use `skills/shared/datamapper-canonicalize.md` to select inline Groovy or XSLT
  and append its exact canonical
  `### DataMapper: kaoto-datamapper-{id}` section. Do not ask ad-hoc mapping questions here.
...
> `camel-implement` uses to generate the selected inline Groovy or XSLT transformation.
...
> Insert the exact non-empty canonical section emitted by `guides/biztalk-map-conversion.md` and

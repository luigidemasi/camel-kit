...
| 4 | Is something broken — build fails, startup errors, runtime exceptions? | Fix it | `/camel-verify` |
...
- "It was working but now it's broken" → go to `/camel-verify`
...
Both pipelines share plan → execute → verify. Only the entry point differs.
...
| `/camel-verify` | Quality analysis without running the app (use `/camel-validate`) |
...
|---|---|
| `/camel-validate` | Static quality analysis of existing routes — generates a report without building/running |

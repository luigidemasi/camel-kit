...
The optional Ship controller requires Linux and supports Camel Main projects with YAML DSL routes. Its default backend requires Pi and Node. Bob 2 and GitHub Copilot CLI can use their own read-only native subagents through the generated Ship skill; the controller writes their proposals and validates the result. See [Bob native Ship](docs/ship-native.md) and [Copilot native Ship](docs/ship-copilot.md). The maintained versions are Pi `0.84.2` or `0.83.0` with Node `22.22.2`; unverified versions run only with `--accept-experimental`, while explicitly incompatible versions remain rejected. See [`camel-ship`](docs/commands.md#camel-ship) for the complete contract.

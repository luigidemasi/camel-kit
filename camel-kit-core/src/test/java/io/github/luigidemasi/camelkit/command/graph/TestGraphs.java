...
public final class TestGraphs {
    ...
    public static ProjectGraph sampleProject() {
        ...
        graph.addNode(new GraphNode("route:order-process", NodeType.CAMEL_ROUTE,
                Map.of("name", "order-process", "from", "kafka:orders-in")));
        graph.addNode(new GraphNode("route:order-validate", NodeType.CAMEL_ROUTE,
                Map.of("name", "order-validate", "from", "direct:validate")));

        graph.addNode(new GraphNode("endpoint:direct:orders", NodeType.CAMEL_ENDPOINT,
                Map.of("uri", "direct:orders", "scheme", "direct")));
        graph.addNode(new GraphNode("endpoint:kafka:orders-in", NodeType.CAMEL_ENDPOINT,
                Map.of("uri", "kafka:orders-in", "scheme", "kafka")));
        graph.addNode(new GraphNode("endpoint:kafka:orders-out", NodeType.CAMEL_ENDPOINT,
                Map.of("uri", "kafka:orders-out", "scheme", "kafka")));
        graph.addNode(new GraphNode("endpoint:log:processed", NodeType.CAMEL_ENDPOINT,
        ...
        graph.addNode(new GraphNode("prop:app.kafka.group-id", NodeType.CONFIG_PROPERTY,
                Map.of("key", "app.kafka.group-id", "value", "order-service")));

        graph.addNode(new GraphNode("class:OrderProcessor", NodeType.CLASS,
                Map.of("fqn", "com.example.OrderProcessor", "name", "OrderProcessor")));
        graph.addNode(new GraphNode("resource:order-routes.camel.yaml", NodeType.RESOURCE_FILE,
        ...
        graph.addEdge(new GraphEdge("endpoint:kafka:orders-in", "maven:camel-kafka", EdgeType.USES_COMPONENT, Map.of()));
        graph.addEdge(new GraphEdge("endpoint:kafka:orders-out", "maven:camel-kafka", EdgeType.USES_COMPONENT, Map.of()));

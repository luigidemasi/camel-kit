...
description: |
  ...
  Reads the TDD to select which Critic Lanes to activate, dispatches each as a
...
1. **Phase 1 — Lane Selection & Dispatch:** Read the TDD and implementer output, decide which Critic Lanes to activate, build each critic's prompt, dispatch all critics in parallel
...
| Security | `agents/critic-security.md` | TDD mentions external boundaries, credentials, API keys, or task crosses a network boundary (`enrich`, `pollEnrich`, HTTP/REST, message brokers) |
| Performance | `agents/critic-performance.md` | TDD mentions high throughput, large payloads, aggregation, polling, or batch processing |
...
- Scan the TDD for activation signals — look for keywords, conditional sections, and component types
...
- Matching on root cause (e.g., Security flags "hardcoded credential" and Route Architecture flags "option value doesn't match TDD" for the same line)
...
1. [Lane: X] [finding description] — [file:location]
   Dismissal evidence: [TDD section or code reference]
2. ...

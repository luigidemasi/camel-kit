...
Document in the design spec under the **"Resilience / Circuit Breaker"** section (load `guides/performance.md` for circuit breaker configuration reference).
...
Document in the design spec under the **"Idempotent Consumer"** section.
...
Document in the design spec under the **"Transactions"** section. Propagation policies: `PROPAGATION_REQUIRED`
(default — join or create), `PROPAGATION_REQUIRES_NEW` (always new), `PROPAGATION_MANDATORY` (must exist). Combine
with `onException` + `markRollbackOnly` for rollback on specific exceptions.

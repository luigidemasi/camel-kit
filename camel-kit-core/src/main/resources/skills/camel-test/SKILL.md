...
and best practices. If Citrus MCP is unavailable, fall back only to the cached quick reference for the same
`citrus.version` configured in `.camel-kit/config.properties`.

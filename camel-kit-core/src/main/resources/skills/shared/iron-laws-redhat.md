...
> Five non-negotiable laws enforced across ALL pipeline phases. No exceptions. No workarounds. No "just this once."
...
```
ONLY RED HAT SUPPORTED CAMEL VERSIONS AND COMPONENTS.
COMMUNITY-ONLY VERSIONS ARE FORBIDDEN.
```

Camel-Kit generates production integrations for Red Hat customers. Every version must come from `maven.repository.redhat.com`, every component must be verified as supported.

**Gate function:**
1. CHECK the project's configured Camel version has a `.redhat-XXXXX` suffix
2. VERIFY the version exists in `catalog/versions.properties`
3. For MCP calls, STRIP the `.redhat-XXXXX` suffix (MCP catalog uses community version numbers)
4. VERIFY component support status via `camel_rh_build_component_info` (camel-knowledge MCP)
5. Only "Production Support" components are allowed without warning

**Support levels:**
- **Production Support** — fully supported, use freely
- **Technology Preview** — WARN the user, suggest alternative
- **Community Support / Dev Support** — WARN the user, strongly suggest alternative
- **Not found** — REJECT, find a supported alternative

### Rationalization Table

| Excuse | Reality |
|--------|---------|
| "The community version works fine" | Works ≠ supported. Customer needs support contracts. |
| "I'll let the user decide the version" | Present Red Hat versions. Never suggest community-only. |
| "This component works, just not in the Red Hat build" | Not supported = not an option for production. |
| "The latest community version has better features" | Features ≠ support. Red Hat versions are the constraint. |

### Red Flags — STOP If You Think:

- "Let me just use the latest Camel version..."
- "The community catalog has this component..."
- "Red Hat probably supports this..."
- "I'll check support status later..."

---

## Iron Law 3: Constitution Compliance
...
7. **Component Support Verification** — every component verified as Red Hat supported (see Iron Law 2).
...
8. CHECK Rule 7: all components MCP-verified as Red Hat supported?
...
## Iron Law 4: No Code Without Spec Approval
...
## Iron Law 5: Spec Compliance Before Quality

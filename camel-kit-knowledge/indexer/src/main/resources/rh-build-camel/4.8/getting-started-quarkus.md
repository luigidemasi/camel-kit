...
2. [Preface](#idm139659010523200)
3. 1. Getting Started with Red Hat build of Apache Camel for Quarkus
4. [2. Deploying Quarkus applications](#deploying-quarkus-applications)
5. 3. Setting up Maven locally
6. 4. Sample applications
7. [Legal Notice](#idm139659010724592)
...
[Legal Notice](#idm139659010724592)
...
## [Preface Copy link](#idm139659010523200)
...
## [Chapter 1. Getting Started with Red Hat build of Apache Camel for Quarkus Copy link](#getting-started-with-camel-quarkus-extensions_camel-quarkus-extensions-getting-started)
...
### [1.1. Red Hat build of Apache Camel for Quarkus overview Copy link](#camel-quarkus-overview)
...
#### [1.1.1. Languages Copy link](#languages)
...
### [1.2. Tooling Copy link](#tooling)

#### [1.2.1. IDE plugins Copy link](#first-steps-with-camel-extensions-for-quarkus)
...
- [VS Code extension](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-quarkus)
- [Eclipse plugin](https://marketplace.eclipse.org/content/quarkus-tools) (currently not supported)
- [IntelliJ plugin](https://plugins.jetbrains.com/plugin/13234-quarkus-tools) (currently not supported)

Check the plugin documentation to discover how to create projects for your preferred IDE.

#### [1.2.2. Camel content assist Copy link](#camel_content_assist)
...
- [Debug Adapter for Apache Camel](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-debug-adapter-apache-camel) to debug Camel integrations written in Java, YAML or XML locally.
- [Eclipse Desktop Language Support for Camel](https://marketplace.eclipse.org/content/language-support-apache-camel) - a part of [Jboss Tools](https://tools.jboss.org/)
...
### [1.3. Building your first project with Red Hat build of Apache Camel for Quarkus Copy link](#building-your-first-project-with-camel-extensions-for-quarkus)

#### [1.3.1. Overview Copy link](#overview)
...
#### [1.3.2. Generating the skeleton application with code.quarkus.redhat.com Copy link](#generating_the_skeleton_application_with_code_quarkus_redhat_com)
...
#### [1.3.3. Explore the application code Copy link](#explore_the_application_code)
...
#### [1.3.4. Adding a simple Camel route Copy link](#adding_a_simple_camel_route)
...
#### [1.3.5. Development mode Copy link](#development_mode)
...
#### [1.3.6. Testing Copy link](#testing-your-first-camel-extensions-for-quarkus-project)

##### [1.3.6.1. JVM mode Copy link](#jvm_mode)
...
##### [1.3.6.2. Native mode Copy link](#native_mode)
...
2. Add the `NativeRoutesIT` class as shown in the following code snippet: **NativeRoutesIT.java** `package org.acme; import io.quarkus.test.junit.NativeImageTest; @NativeImageTest public class NativeRoutesIT extends RoutesTest { // Execute the same tests but in native mode. }` Copy to Clipboard Toggle word wrap The native mode tests are verified by `maven-failsafe-plugin` in the `verify` phase.
3. Pass the `native` property to activate the profile that runs them: `$ mvn clean verify -Pnative` Copy to Clipboard Toggle word wrap

Tip

For more details, and how to use the `CamelTestSupport` style of testing, see [Testing Camel Quarkus Extensions](#camel-quarkus-cxf-testing-guide) .

#### [1.3.7. Packaging and running the application Copy link](#packaging-your-first-camel-extensions-for-quarkus-project)

##### [1.3.7.1. JVM mode Copy link](#jvm_mode_2)
...
##### [1.3.7.2. Native mode Copy link](#native_mode_2)
...
#### [1.4.1. Running in JVM mode Copy link](#running_in_jvm_mode)
...
#### [1.4.2. Running in native mode Copy link](#native-tests)
...
#### [1.4.3. Differences between @QuarkusTest and @QuarkusIntegrationTest Copy link](#jvm-vs-native-tests)
...
##### [1.4.3.1. @QuarkusTest in JVM mode Copy link](#literal_quarkustest_literal_in_jvm_mode)
...
##### [1.4.3.2. @QuarkusIntegrationTest in native mode Copy link](#literal_quarkusintegrationtest_literal_in_native_mode)
...
#### [1.4.4. Testing with external services Copy link](#testing_with_external_services)

##### [1.4.4.1. Testcontainers Copy link](#testcontainers)
...
##### [1.4.4.1.1. Passing configuration data with QuarkusTestResourceLifecycleManager Copy link](#passing_configuration_data_with_literal_quarkustestresourcelifecyclemanager_literal)
...
##### [1.4.4.2. WireMock Copy link](#wiremock)
...
##### [1.4.4.2.1. Setting up WireMock Copy link](#setting_up_wiremock)
...
#### [1.4.5. CamelTestSupport style of testing with CamelQuarkusTestSupport Copy link](#cameltestsupport_style_of_testing)
...
##### [1.4.5.1. Testing with CamelQuarkusTestSupport in JVM mode Copy link](#testing_with_literal_camelquarkustestsupport_literal_in_jvm_mode)
...
##### [1.4.5.2. Customizing the CamelContext for testing Copy link](#customizing_the_literal_camelcontext_literal_for_testing)
...
##### [1.4.5.3. Configuring routes for testing Copy link](#configuring_routes_for_testing)
...
##### [1.4.5.4. CamelContext test lifecycle Copy link](#camelcontext_test_lifecycle)
...
##### [1.4.5.5. Examples Copy link](#examples)

##### [1.4.5.5.1. Simple RouteBuilder and test class Copy link](#simple_literal_routebuilder_literal_and_test_class)
...
##### [1.4.5.5.2. Using AdviceWith Copy link](#using_literal_advicewith_literal)
...
##### [1.4.5.5.3. Explicitly enabling advice Copy link](#explicitly_enabling_advice)

When explicitly [enabling advice](https://rhaetor.github.io/rh-camel/manual/advice-with.html#_enabling_advice_during_testing) you must invoke `startRouteDefinitions` when completing your `AdviceWith` setup.

Note

Invoking `startRouteDefinitions` is only required if you have routes configured that are **NOT** being advised.

##### [1.4.5.6. Limitations Copy link](#limitations)

##### [1.4.5.6.1. Test lifecycle methods inherited from CamelTestSupport Copy link](#test_lifecycle_methods_inherited_from_literal_cameltestsupport_literal)
...
##### [1.4.5.6.2. Creating a custom Camel registry is not supported Copy link](#creating_a_custom_camel_registry_is_not_supported)
...
## [Chapter 2. Deploying Quarkus applications Copy link](#deploying-quarkus-applications)
...
## [Chapter 3. Setting up Maven locally Copy link](#set-up-maven-locally)

Typical Red Hat build of Apache Camel application development uses Maven to build and manage projects.

### [3.1. Preparing to set up Maven Copy link](#prepare-to-set-up-maven)
...
2. Ensure that your system is connected to the Internet. While building a project, the default behavior is that Maven searches external repositories and downloads the required artifacts. Maven looks for repositories that are accessible over the Internet. You can change this behavior so that Maven searches only repositories that are on a local network. That is, Maven can run in an offline mode. In offline mode, Maven looks for artifacts in its local repository. See [Section 3.3, "Using local Maven repositories"](#use-local-maven-repositories) .

### [3.2. Adding Red Hat repositories to Maven Copy link](#add-red-hat-repositories-to-maven)
...
### [3.3. Using local Maven repositories Copy link](#use-local-maven-repositories)
...
### [3.4. Setting Maven mirror using environmental variables or system properties Copy link](#set-maven-mirror-url)
...
#### [3.4.1. About Maven mirror Copy link](#maven-mirror)
...
#### [3.4.2. Adding Maven mirror to settings.xml Copy link](#add-maven-mirror-url-settings-xml)
...
#### [3.4.3. Setting Maven mirror using environmental variable or system property Copy link](#set-maven-mirror-url-using-env-variables)
...
#### [3.4.4. Using Maven options to specify Maven mirror url Copy link](#set-maven-mirror-url-using-maven-options)
...
### [3.5. About Maven artifacts and coordinates Copy link](#about-maven-coordinates)
...
## [Chapter 4. Sample applications Copy link](#camel-quarkus-examples_cq-examples)

### [4.1. Red Hat build of Quarkus Examples Copy link](#camel-quarkus-examples)

The [Red Hat build of Quarkus examples](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/) repository contains a number of examples on how to integrate with Camel for a variety of use cases. They provide best practice advice and describe common patterns that we see in integration and messaging problems.
...
### [4.2. Examples repository Copy link](#camel-quarkus-examples-list)

Expand

| Example                                                                                                                                                                                                | Description                                                                                                                       |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|
| [Artemis to ElasticSearch](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/artemis-elasticsearch)                                                                             | Shows how the message is consumed from the Apache Artemis broker using MQTT protocol, transformed, and loaded into ElasticSearch. |
| [Leader election in Kubernetes](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/cluster-leader-election)                                                                      | Shows how to use Camel's master component for Kubernetes leader election.                                                         |
| [Custom](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/timer-log-main)  [`main()`](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/timer-log-main) | Shows how to start Camel from a custom  ``` main() ```  method.                                                                   |
| [Camel Quarkus CXF SOAP example](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/cxf-soap)                                                                                    | Shows how to use Camel CXF SOAP component.                                                                                        |
| [Extract, Transform, and Load between DBs](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/jdbc-datasource)                                                                   | Shows how to extract, transform, and load between two databases.                                                                  |
| [File consumer with Bindy &amp; FTP](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/file-bindy-ftp)                                                                          | Shows how to consume CSV files, marshal & unmarshal the data, and send it via FTP.                                                |
| [Health](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/health)                                                                                                              | Shows how to use Camel health-checks with Quarkus.                                                                                |
| [HTTP with vanilla JAX-RS or Camel HTTP](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/http-log)                                                                            | Shows how to create HTTP endpoints using either platform-http or RESTEasy.                                                        |
| [JMS and JPA](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/jms-jpa)                                                                                                        | Demonstrates a Camel Quarkus application supporting JTA transactions across multiple transactional resources.                     |
| [JPA idempotent repository](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/jpa-idempotent)                                                                                   | Shows how to consume a message only once, even when delivered multiple times.                                                     |
| [JTA and JPA](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/jta-jpa)                                                                                                        | Demonstrates JTA transactions across MySQL and a simulated XAResource.                                                            |
| [Kafka example](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/kafka)                                                                                                        | Shows how to produce and consume messages in a Kafka topic using Strimzi Operator.                                                |
| [Kamelet Chuck Norris](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/kamelet-chuck-norris)                                                                                  | Shows how to build a simple Kamelet for use in Camel applications.                                                                |
| [Message Bridge](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/message-bridge)                                                                                              | Demonstrates AMQ and IBM MQ clients with connection pooling and XA transactions.                                                  |
| [Observability](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/observability)                                                                                                | Demonstrates adding metrics, health checks, and distributed tracing support.                                                      |
| [OpenAPI Contract First](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/openapi-contract-first)                                                                              | Shows how to run with Contract First OpenAPI.                                                                                     |
| [Platform HTTP security with Keycloak](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/platform-http-keycloak)                                                                | Shows how to secure platform HTTP with Keycloak.                                                                                  |
| [REST with Jackson](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/rest-json)                                                                                                | Demonstrates creating a REST service using Camel REST DSL and Jackson.                                                            |
| [Saga and LRA](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/saga)                                                                                                          | Shows how to use saga and LRA patterns.                                                                                           |
| [Timer to Log](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/timer-log)                                                                                                     | Uses the Camel timer component to output a Hello world message to the console.                                                    |
| [Tokenize a CSV file](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/file-split-log-xml)                                                                                     | Shows how to define a Camel route in XML for tokenizing a CSV file.                                                               |
| [Vertx-Websocket Chat](https://github.com/jboss-fuse/camel-quarkus-examples/tree/3.15.0-product/vertx-websocket-chat)                                                                                  | Shows how to configure a WebSocket server and interact with connected peers.                                                      |

Show more

## [Legal Notice Copy link](#idm139659010724592)

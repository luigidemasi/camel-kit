...
public class GraphProjectNormsCommand extends GraphQueryCommand {
    ...
    protected String execute(ProjectGraph graph) {
        ...
        List<String> ids
                = routes.stream().map(r -> r.properties().getOrDefault("name", r.id())).collect(Collectors.toList());
        ...
        long matchCount
                = ids.stream().filter(n -> (pattern.equals("kebab-case") ? KEBAB : CAMEL).matcher(n).matches()).count();
        ...
        for (GraphNode route : routes) {
            ...
            if (nb.nodes().stream().anyMatch(n -> {
                String id = n.id().toLowerCase(Locale.ROOT);
                return id.contains("error") || id.contains("exception") || id.contains("dlq");
            }))
                withEH++;
        }
        ...
        props.forEach(p -> {
            String key = p.properties().getOrDefault("key", p.id());

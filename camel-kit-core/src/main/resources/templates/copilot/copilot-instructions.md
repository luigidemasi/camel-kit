...
eligible native stages use the dedicated read-only `camel-ship-worker`, while the CLI owns oversight, catalog checks,
candidate writes, validation and publication. Do not route Ship through the manual pipeline skills or substitute another
custom agent.

...
2. [Preface](#idm140330158409360)
3. [1. Red Hat build of Apache Camel supported integrations](#available-integrations)
4. 2. Tooling Guide
...
9. 7. Using Camel CLI with Red Hat build of Apache Camel for Spring Boot
10. [Legal Notice](#idm140330149382432)
...
[Legal Notice](#idm140330149382432)

**Abstract**

The Tooling Guide for Red Hat build of Apache Camel introduces tooling extensions that are used for Red Hat build of Apache Camel.

## [Preface Copy link](#idm140330158409360)
...
## [Chapter 1. Red Hat build of Apache Camel supported integrations Copy link](#available-integrations)
...
### [4.1. Language support for Apache Camel extension Copy link](#vscode-language-support-extension-language)
...
- The Kamelet catalog used is community supported version only. For the list of supported Kamelets, see [kamelets-reference-name](https://content-preview.docs.redhat.com/en/documentation/red_hat_build_of_apache_camel/4.14/html-single/kamelets_reference_for_red_hat_build_of_apache_camel/index) .
...
### [5.1. Debug Adapter for Apache Camel routes Copy link](#vscode-debug-adapter-extension-debug)
...
### [5.2. Using VSCode Debug Adapter for Apache Camel with standalone Camel routes Copy link](#using-vscode-debug-adapter-camel-debug)
...
### [5.3. Using VSCode Debug Adapter for Apache Camel with Quarkus Copy link](#using-vscode-debug-adapter-cq-debug)
...
### [5.4. Using VSCode Debug Adapter for Apache Camel with Spring Boot Copy link](#using-vscode-debug-adapter-sb-debug)
...
1. JBang must be installed on your machine. See [instructions](https://www.jbang.dev/download/) on how to download and install JBang.
...
2. Run the following command to install the Camel CLI application: `jbang app install camel@redhat-camel/jbang-catalog/4.14.2` Copy to Clipboard Toggle word wrap
...
#### [6.1.1. Adding Red Hat repositories to Maven Copy link](#add-red-hat-repositories-to-maven-cq)
...
#### [6.2.1. Enable shell completion Copy link](#enable_shell_completion)
...
When you use the init command, Camel by default creates the file in the current directory. You can use the `--directory` option to create the file in the specified directory. For example to create in a folder named `foobar` , run:
...
##### [6.3.16.1. Top status of the Camel integrations Copy link](#top_status_of_the_camel_integrations)
...
##### [6.3.17.1. Using stream:in with line vs raw mode Copy link](#using_streamin_with_line_vs_raw_mode)
...
#### [6.3.19. Using the platform-http component Copy link](#using_the_platform_http_component)
...
org.apache.camel.quarkus:camel-quarkus-core:3.27
org.apache.camel.quarkus:camel-quarkus-debug:3.27
org.apache.camel.quarkus:camel-quarkus-microprofile-health:3.27
org.apache.camel.quarkus:camel-quarkus-platform-http:3.27
org.apache.camel.quarkus:camel-timer:3.27
...
When exporting, the Camel version defined in the `pom.xml` or `build.gradle` is the same version as Camel CLI uses. You can specify the different Camel version as shown:

```
camel export --runtime=quarkus --gav=com.foo:acme:1.0-SNAPSHOT --directory=../myproject --quarkus-version=3.27.2.redhat-00002
```
...
#### [6.8.3. Exporting with verbose output Copy link](#exporting_with_verbose_output)

If you need additional logging you can use the `--verbose` option:

```
camel export --verbose --runtime=spring-boot --gav=com.foo:acme:1.0-SNAPSHOT --dep=camel:cli-connector --directory=../myproject
```
...
This will show the full output of the export command.

#### [6.8.4. Configuring the export Copy link](#configuring_the_export)
...
#### [6.8.5. Configuration Copy link](#configuration)
...
##### [6.8.5.1. Set and unset configuration Copy link](#set_and_unset_configuration)
...
camel config set camel-quarkus-version=3.27
```
...
##### [6.8.5.2. List and get configurations Copy link](#list_and_get_configurations)
...
##### [6.8.5.3. Placeholders substitutes Copy link](#placeholders_substitutes)
...
## [Chapter 7. Using Camel CLI with Red Hat build of Apache Camel for Spring Boot Copy link](#camel-cli-sb)
...
#### [7.1.1. Adding Red Hat repositories to Maven Copy link](#add-red-hat-repositories-to-maven-sb)
...
#### [7.2.1. Enable shell completion Copy link](#enable_shell_completion_2)
...
### [7.3. Upgrading Red Hat build of Apache Camel on Spring Boot version with Camel update recipes Copy link](#csb-camel-upgrade-recipes)
...
#### [7.3.1. Running Camel Updates Copy link](#running_camel_updates)

To perform the update to this version, use:

```
$ camel update run 4.14 .2.redhat-00014 --runtime = spring-boot
```
...
##### [7.3.1.1. Configuration Options Copy link](#configuration_options)

You can customize the update process with several options:

&lt;version&gt; The version to which the Camel project should be updated. --camelArtifact The Maven artifact coordinates for the Camel upgrade recipes. --camelSpringBootArtifact The Maven artifact coordinates for the Camel Spring Boot upgrade recipes. --debug Enables debug logging if set to true. --dryRun If set to true, performs a dry run of the update process without making any changes. --extraActiveRecipes Comma separated list of recipes to be executed after the Camel one, make sure the artifact containing the recipes is added via extraRecipeArtifactCoordinates --extraRecipeArtifactCoordinates Comma separated list of artifact coordinates containing extraActiveRecipes, ex.my.org:recipes: 1.0.0 -h, --help Display the help and sub-commands --openRewriteVersion The version of OpenRewrite to use during the update process. --quarkusMavenPluginGroupId The group ID of the Quarkus Maven plugin. --quarkusMavenPluginVersion The version of the Quarkus Maven plugin to use. --repos Additional maven repositories for download on-demand (Use commas to separate multiple repositories) --runtime Runtime ( `camel-main` , `spring-boot` , `quarkus` ) --upgradeTimeout Time to wait, in seconds, before shutting down the upgrade process

##### [7.3.1.2. Updating a plain Camel application Copy link](#updating_a_plain_camel_application)
...
##### [7.3.1.3. Updating a Spring Boot application Copy link](#updating_a_spring_boot_application)
...
### [7.4. Creating and running Camel routes Copy link](#camel-jbang-running-camel-routes-sb)
...
#### [7.4.1. Running routes from multiple files Copy link](#running_routes_from_multiple_files_2)
...
#### [7.4.2. Running routes from input parameter Copy link](#running_routes_from_input_parameter_2)
...
#### [7.4.3. Dev mode with live reload Copy link](#dev_mode_with_live_reload_2)
...
#### [7.4.4. Developer Console Copy link](#developer_console_2)
...
#### [7.4.5. Using profiles Copy link](#using_profiles_2)
...
In the `properties` files you can configure all configurations from [Camel Main](https://camel.apache.org/components/4.14.1/others/main.html) . To turn off and enable log masking run the following command:
...
#### [7.4.6. Downloading JARs over the internet Copy link](#downloading_jars_over_the_internet_2)
...
#### [7.4.7. Adding custom JARs Copy link](#adding_custom_jars_2)
...
#### [7.4.8. Using 3rd-party Maven repositories Copy link](#using_3rd_party_maven_repositories_2)
...
#### [7.4.9. Configuration of Maven usage Copy link](#configuration_of_maven_usage_2)
...
#### [7.4.10. Running routes hosted on GitHub Copy link](#running_routes_hosted_on_github_2)
...
##### [7.4.10.1. Running routes from the GitHub gists Copy link](#running_routes_from_the_github_gists_2)
...
#### [7.4.11. Downloading routes hosted on the GitHub Copy link](#downloading_routes_hosted_on_the_github_2)
...
##### [7.4.11.1. Downloading routes form GitHub gists Copy link](#downloading_routes_form_github_gists_2)
...
#### [7.4.12. Running the Camel K integrations or bindings Copy link](#running_the_camel_k_integrations_or_bindings_2)
...
#### [7.4.13. Run from the clipboard Copy link](#run_from_the_clipboard_2)
...
#### [7.4.14. Controlling the local Camel integrations Copy link](#controlling_the_local_camel_integrations_2)
...
#### [7.4.15. Controlling the Spring Boot and Quarkus integrations Copy link](#controlling_the_spring_boot_and_quarkus_integrations)
...
#### [7.4.16. Getting the status of Camel integrations Copy link](#getting_the_status_of_camel_integrations_2)
...
camel get
  PID   NAME      CAMEL            PLATFORM            READY  STATUS    AGE    TOTAL  FAILED  INFLIGHT  SINCE-LAST
 61818  MyCamel   4.14.1-SNAPSHOT  Spring Boot v2.7.3   1/1   Running  28m34s    854       0         0     0s/0s/-
 63051  test1     4.14.1-SNAPSHOT  JBang                1/1   Running     18s     14       0         0     0s/0s/-
 63068  mygroovy  4.14.1-SNAPSHOT  JBang                1/1   Running      5s      2       0         0     0s/0s/-
```
...
##### [7.4.16.1. Top status of the Camel integrations Copy link](#top_status_of_the_camel_integrations_2)
...
camel top
  PID   NAME     JAVA     CAMEL            PLATFORM            STATUS    AGE         HEAP        NON-HEAP     GC     THREADS   CLASSES
 22104  chuck    11.0.13  4.14.1-SNAPSHOT  JBang               Running   2m10s  131/322/4294 MB  70/73 MB  17ms (6)      7/8  7456/7456
 14242  MyCamel  11.0.13  4.14.1-SNAPSHOT  Spring Boot v2.7.3  Running  33m40s  115/332/4294 MB  62/66 MB  37ms (6)    16/16  8428/8428
 22116  bar      11.0.13  4.14.1-SNAPSHOT  JBang               Running    2m7s   33/268/4294 MB  54/58 MB  20ms (4)      7/8  6104/6104
```
...
##### [7.4.16.2. Starting and Stopping the routes Copy link](#starting_and_stopping_the_routes_2)
...
##### [7.4.16.3. Configuring the logging levels Copy link](#configuring_the_logging_levels_2)
...
##### [7.4.16.4. Listing services Copy link](#listing_services_2)
...
##### [7.4.16.4.1. Listing state of Circuit Breakers Copy link](#listing_state_of_circuit_breakers_2)

If your Camel integration uses the [Circuit Breaker](https://camel.apache.org/components/4.14.x/eips/circuitBreaker-eip.html) , then you can output the status of the breakers with Camel CLI as follows:
...
#### [7.4.17. Scripting from the terminal using pipes Copy link](#scripting_from_the_terminal_using_pipes_2)
...
##### [7.4.17.1. Using stream:in with line vs raw mode Copy link](#using_streamin_with_line_vs_raw_mode_2)

When using `stream:in` to read data from `System in` then the [Stream Component](https://camel.apache.org/components/4.14.x/stream-component.html) works in two modes:
...
#### [7.4.18. Running local Kamelets Copy link](#running_local_kamelets_2)
...
#### [7.4.19. Using the platform-http component Copy link](#using_the_platform_http_component_2)
...
#### [7.4.20. Using Java beans and processors Copy link](#using_java_beans_and_processors_2)
...
#### [7.4.21. Dependency Injection in Java classes Copy link](#dependency_injection_in_java_classes)
...
##### [7.4.21.1. Using Spring Boot dependency injection Copy link](#using_spring_boot_dependency_injection)
...
#### [7.4.22. Debugging Copy link](#debugging_2)
...
##### [7.4.22.1. Java debugging Copy link](#java_debugging_2)
...
##### [7.4.22.2. Camel route debugging Copy link](#camel_route_debugging_2)

The Camel route debugger is available by default (the `camel-debug` component is automatically added to the classpath). By default, it can be reached through JMX at the URL `service:jmx:rmi:///jndi/rmi://localhost:1099/jmxrmi/camel` . You can then use the Integrated Development Environment (IDE) of your choice.

#### [7.4.23. Health Checks Copy link](#health_checks_2)
...
### [7.5. Listing what Camel components is available Copy link](#camel-jbang-listing-camel-components-sb)
...
#### [7.5.1. Displaying component documentation Copy link](#displaying_component_documentation_2)
...
##### [7.5.1.1. Browsing online documentation from the Camel website Copy link](#browsing_online_documentation_from_the_camel_website_2)
...
##### [7.5.1.2. Filtering options listed in the tables Copy link](#filtering_options_listed_in_the_tables_2)
...
### [7.6. Gathering list of dependencies Copy link](#camel-jbang-list-of-dependencies-sb)
...
org.apache.camel.springboot:camel-spring-boot-engine-starter:4.14.2
org.apache.camel.springboot:camel-dsl-modeline-starter:4.14.2
org.apache.camel.springboot:camel-kamelet-starter:4.14.2
org.apache.camel.springboot:camel-log-starter:4.14.2
org.apache.camel.springboot:camel-rest-starter:4.14.2
org.apache.camel.springboot:camel-stream-starter:4.14.2
org.apache.camel.springboot:camel-timer-starter:4.14.2
...
### [7.7. Open API Copy link](#camel-jbang-open-api-sb)

Camel CLI allows to quickly expose an Open API service using `contract first` approach, where you have an existing OpenAPI specification file. Camel CLI bridges each API endpoints from the OpenAPI specification to a Camel route with the naming convention `direct:<operationId>` . This make it quicker to implement a Camel route for a given operation.

See the [OpenAPI example](https://github.com/apache/camel-kamelets-examples/tree/main/jbang/open-api) for more details.

### [7.8. Troubleshooting Copy link](#camel-jbang-troubleshooting-sb)

When you use JBang, it stores the state in `~/.jbang` directory. This is also the location where JBang stores downloaded JARs. Camel CLI also downloads the needed dependencies while running.

However, these dependencies are downloaded to your local Maven repository `~/.m2` . So when you troubleshoot the problems such as an outdated JAR while running the Camel CLI, try to delete these directories, or parts of it.

### [7.9. Exporting to Red Hat build of Apache Camel for Spring Boot Copy link](#camel-jbang-exporting-to-camel-spring-boot-sb)
...
#### [7.9.1. Exporting to Red Hat build of Apache Camel for Spring Boot Copy link](#exporting_to_red_hat_build_of_apache_camel_for_spring_boot)
...
When exporting to the Spring Boot, the Camel version defined in the `pom.xml` or `build.gradle` is the same version as Camel CLI uses. You can specify a different Camel version as shown:

```
camel export --runtime=spring-boot --gav=com.foo:acme:1.0-SNAPSHOT --directory=../myproject --camel-spring-boot-version=4.14.2.redhat-00018
```
...
#### [7.9.2. Exporting with Camel CLI included Copy link](#exporting_with_camel_cli_included_2)
...
#### [7.9.3. Exporting with verbose output Copy link](#exporting_with_verbose_output_2)
...
#### [7.9.4. Configuring the export Copy link](#configuring_the_export_2)
...
#### [7.9.5. Configuration Copy link](#configuration_2)
...
##### [7.9.5.1. Set and unset configuration Copy link](#set_and_unset_configuration_2)
...
camel config set build-property=openshift-maven-plugin-version=1.18.2.redhat-00018
...
##### [7.9.5.2. List and get configurations Copy link](#list_and_get_configurations_2)
...
##### [7.9.5.3. Placeholders substitutes Copy link](#placeholders_substitutes_2)
...
#### [7.9.6. Troubleshooting Copy link](#troubleshooting)
...
## [Legal Notice Copy link](#idm140330149382432)

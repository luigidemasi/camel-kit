...
> This reference provides actionable guidance for applying design patterns in Apache Camel integrations.
...
| `PROPAGATION_REQUIRES_NEW` | Always create a new transaction, suspend existing |
...
- onException:
    ...
    handled: true
    markRollbackOnly: true  # Rollback transaction
    steps:
...
- Use `markRollbackOnly` in `onException` to rollback on specific exceptions
...
| `N replicas × consumersCount` | Total consumers in cluster |
| `consumers > partitions` | Starving consumers (some idle) |
...
```yaml
# Use consumersCount=1 and scale via replicas
- route:
    id: scalable-consumer
    from:
      uri: kafka:orders
      parameters:
        groupId: order-processors
        consumersCount: 1  # Scale via pod replicas
...
| Strategy | Use When |
|----------|----------|
| `earliest` | Process all available messages from beginning |
| `latest` | Only process new messages after consumer starts |
...
- [Apache Camel EIP Documentation](https://camel.apache.org/components/4.14.x/eips/enterprise-integration-patterns.html)

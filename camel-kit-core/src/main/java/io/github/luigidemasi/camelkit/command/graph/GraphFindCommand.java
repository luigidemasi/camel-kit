...
public class GraphFindCommand extends GraphQueryCommand {
    @Option(names = {"--type"}, description = "Node type filter") String type;
    @Option(names = {"--name"}, description = "Name pattern (regex)", defaultValue = ".*") String namePattern;
    ...
    @Override protected String execute(ProjectGraph graph) {
        ...
        if (type != null) {
            try { nodeType = NodeType.valueOf(type); }
                ...
                error.put("validTypes", Arrays.stream(NodeType.values()).map(Enum::name).collect(Collectors.joining(", ")));

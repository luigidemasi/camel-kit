...
2. [Preface](#idm140462621926160)
3. 1. Tooling Guide
4. 2. Tooling Guide extension pack
5. 3. Language support for Camel
6. 4. Debug support for Camel
7. 5. Using Camel CLI with Red Hat build of Apache Camel for Quarkus
8. 6. Using Camel CLI with Camel Spring Boot
9. [Legal Notice](#idm140462613985360)
...
## Information about coding extensions and CLI for Red Hat build of Apache Camel.

[Legal Notice](#idm140462613985360)

**Abstract**

The Tooling Guide introduces tooling extensions that are used for Red Hat build for Apache Camel.

## [Preface Copy link](#idm140462621926160)
...
## [Chapter 1. Tooling Guide Copy link](#camel-tooling-guide)

### [1.1. About Tooling Guide Copy link](#introduction_tooling_guide-tooling-guide)

This guide introduces VS Code extensions for Red Hat build of Apache Camel and how to install and use Camel CLI.

## [Chapter 2. Tooling Guide extension pack Copy link](#camel-tooling-guide-extension-pack)
...
### [2.1. Installing extension pack for Apache Camel by Red Hat Copy link](#installing-camel-extension-pack-extension-pack)
...
## [Chapter 3. Language support for Camel Copy link](#camel-tooling-guide-language)

Important

The VS Code extensions for Apache Camel are listed as development support. For more information about scope of development support, see [Development Support Scope of Coverage for Red Hat Build of Apache Camel](https://access.redhat.com/articles/7043889) .

### [3.1. Language support for Apache Camel extension Copy link](#vscode-language-support-extension-language)
...
#### [3.1.1. Features of language support for Apache Camel extension Copy link](#features_of_language_support_for_apache_camel_extension)
...
#### [3.1.2. Requirements Copy link](#requirements)
...
#### [3.1.3. Installing Language support for Apache Camel extension Copy link](#installing_language_support_for_apache_camel_extension)
...
#### [3.1.4. Using specific Camel catalog version Copy link](#using_specific_camel_catalog_version)
...
#### [3.1.5. Limitations Copy link](#limitations)
...
## [Chapter 4. Debug support for Camel Copy link](#camel-tooling-guide-debug)

Important

The VS Code extensions for Apache Camel are listed as development support. For more information about scope of development support, see [Development Support Scope of Coverage for Red Hat Build of Apache Camel](https://access.redhat.com/articles/7043889) .

### [4.1. Debug Adapter for Apache Camel routes Copy link](#vscode-debug-adapter-extension-debug)
...
#### [4.1.1. Features of Debug Adapter Copy link](#features_of_debug_adapter)
...
#### [4.1.2. Requirements Copy link](#requirements_2)
...
For some features, JBang must be available in the CLI environment.

#### [4.1.3. Installing VS Code Debug Adapter for Apache Camel Copy link](#installing_vs_code_debug_adapter_for_apache_camel)
...
### [4.2. Using VSCode Debug Adapter for Apache Camel with standalone Camel routes Copy link](#using-vscode-debug-adapter-camel-debug)
...
### [4.3. Using VSCode Debug Adapter for Apache Camel with Quarkus Copy link](#using-vscode-debug-adapter-cq-debug)
...
### [4.4. Using VSCode Debug Adapter for Apache Camel with Spring Boot Copy link](#using-vscode-debug-adapter-sb-debug)
...
- [NameVSCodeDebug](https://camel-tooling.github.io/camel-dap-client-vscode/)

## [Chapter 5. Using Camel CLI with Red Hat build of Apache Camel for Quarkus Copy link](#camel-cli-cq)

### [5.1. Installing Camel CLI Copy link](#installing-camel-jbang-cq)
...
After you have installed JBang, you can verify JBang is working by executing the following command from a command shell:
...
2. Run the following command to install the Camel CLI application: `jbang app install camel@redhat-camel/jbang-catalog/4.10.3.redhat-00023` Copy to Clipboard Toggle word wrap
...
### [5.2. Using Camel CLI Copy link](#using-camel-jbang-cq)

The Camel CLI supports multiple commands. The `camel help` command can display all available commands.
...
#### [5.2.1. Enable shell completion Copy link](#enable_shell_completion)
...
### [5.3. Creating and running Camel routes Copy link](#camel-jbang-cq-running-camel-routes-cq)
...
#### [5.3.1. Running routes from multiple files Copy link](#running_routes_from_multiple_files)
...
You can use wildcards (i.e. `*` ) to match multiple files, such as running all yaml files:
...
To run all files in the directory, use:
...
#### [5.3.2. Running routes from input parameter Copy link](#running_routes_from_input_parameter)
...
#### [5.3.3. Dev mode with live reload Copy link](#dev_mode_with_live_reload)
...
#### [5.3.4. Developer Console Copy link](#developer_console)
...
#### [5.3.5. Using profiles Copy link](#using_profiles)
...
In the `properties` files you can configure all configurations from [Camel Main](https://camel.apache.org/components/3.20.x/others/main.html) . To turn off and enable log masking run the following command:
...
#### [5.3.6. Downloading JARs over the internet Copy link](#downloading_jars_over_the_internet)
...
4. from all repositories found in active profiles of `~/.m2/settings.xml` or a settings file specified using `--maven-settings` option.
...
#### [5.3.7. Adding custom JARs Copy link](#adding_custom_jars)
...
#### [5.3.8. Using 3rd-party Maven repositories Copy link](#using_3rd_party_maven_repositories)
...
#### [5.3.9. Configuration of Maven usage Copy link](#configuration_of_maven_usage)
...
#### [5.3.10. Running routes hosted on GitHub Copy link](#running_routes_hosted_on_github)
...
You can also use wildcards (i.e. `\*` ) to match multiple files, such as running all groovy files:
...
##### [5.3.10.1. Running routes from the GitHub gists Copy link](#running_routes_from_the_github_gists)
...
#### [5.3.11. Downloading routes hosted on the GitHub Copy link](#downloading_routes_hosted_on_the_github)
...
##### [5.3.11.1. Downloading routes form GitHub gists Copy link](#downloading_routes_form_github_gists)
...
#### [5.3.12. Running the Camel K integrations or bindings Copy link](#running_the_camel_k_integrations_or_bindings)
...
#### [5.3.13. Run from the clipboard Copy link](#run_from_the_clipboard)
...
#### [5.3.14. Controlling the local Camel integrations Copy link](#controlling_the_local_camel_integrations)
...
#### [5.3.15. Controlling Quarkus integrations Copy link](#controlling_quarkus_integrations)
...
#### [5.3.16. Getting the status of Camel integrations Copy link](#getting_the_status_of_camel_integrations)

The `get` command in the Camel CLI is used for getting the Camel specific status for one or all running Camel integrations. To display the status of the running Camel integrations, run:
...
You can also see the status of every routes, from all local Camel integrations with `camel get route` :
...
Use `camel get --help` to display all available commands.

##### [5.3.16.1. Top status of the Camel integrations Copy link](#top_status_of_the_camel_integrations)
...
You can also see the top performing routes (highest to lowest mean processing time) of every routes, from all local Camel integrations with `camel top route` :
...
Use `camel top --help` to display all available commands.

##### [5.3.16.2. Starting and Stopping the routes Copy link](#starting_and_stopping_the_routes)
...
To stop all routes in the `chuck` integration, run:
...
##### [5.3.16.3. Configuring the logging levels Copy link](#configuring_the_logging_levels)
...
##### [5.3.16.4. Listing services Copy link](#listing_services)
...
##### [5.3.16.4.1. Listing state of Circuit Breakers Copy link](#listing_state_of_circuit_breakers)
...
#### [5.3.17. Scripting from the terminal using pipes Copy link](#scripting_from_the_terminal_using_pipes)
...
##### [5.3.17.1. Using stream:in with line vs raw mode Copy link](#using_streamin_with_line_vs_raw_mode)
...
#### [5.3.18. Running local Kamelets Copy link](#running_local_kamelets)
...
#### [5.3.19. Using the platform-http component Copy link](#using_the_platform_http_component)
...
#### [5.3.20. Using Java beans and processors Copy link](#using_java_beans_and_processors)
...
#### [5.3.21. Debugging Copy link](#debugging)
...
##### [5.3.21.1. Java debugging Copy link](#java_debugging)
...
##### [5.3.21.2. Camel route debugging Copy link](#camel_route_debugging)
...
#### [5.3.22. Health Checks Copy link](#health_checks)
...
Use `camel get health --help` to see all various options.

### [5.4. Listing what Camel components is available Copy link](#camel-jbang-cq-listing-camel-components-cq)
...
You can use the Camel CLI to list what Camel provides using the `camel catalog` command. For example, to list all components:
...
#### [5.4.1. Displaying component documentation Copy link](#displaying_component_documentation)
...
##### [5.4.1.1. Browsing online documentation from the Camel website Copy link](#browsing_online_documentation_from_the_camel_website)
...
##### [5.4.1.2. Filtering options listed in the tables Copy link](#filtering_options_listed_in_the_tables)
...
### [5.5. Gathering list of dependencies Copy link](#camel-jbang-cq-list-of-dependencies-cq)
...
```
camel dependency
org.apache.camel:camel-dsl-modeline:4.10.0
org.apache.camel:camel-health:4.10.0
org.apache.camel:camel-kamelet:4.10.0
org.apache.camel:camel-log:4.10.0
org.apache.camel:camel-rest:4.10.0
org.apache.camel:camel-stream:4.10.0
org.apache.camel:camel-timer:4.10.0
org.apache.camel:camel-yaml-dsl:4.10.0
org.apache.camel.kamelets:camel-kamelets-utils:{kamelets-utils-version}
org.apache.camel.kamelets:camel-kamelets:4.10.3
```
...
org.apache.camel.quarkus:camel-quarkus-core:3.20
org.apache.camel.quarkus:camel-quarkus-debug:3.20
org.apache.camel.quarkus:camel-quarkus-microprofile-health:3.20
org.apache.camel.quarkus:camel-quarkus-platform-http:3.20
org.apache.camel.quarkus:camel-timer:3.20
...
### [5.6. Open API Copy link](#camel-jbang-open-api-cq)
...
### [5.7. Troubleshooting Copy link](#camel-jbang-troubleshooting-cq)
...
### [5.8. Exporting to Red Hat build of Apache Camel for Quarkus Copy link](#camel-jbang-cq-exporting-to-camel-quarkus-cq)
...
#### [5.8.1. Exporting to Red Hat build of Apache Camel for Quarkus Copy link](#exporting_to_red_hat_build_of_apache_camel_for_quarkus)
...
For example, to export using

the `quarkus` runtime,

the maven groupID `com.foo` ,

the artifactId `acme` ,

and the version `1.0-SNAPSHOT`

into the `camel-quarkus-jbang` directory,
...
```
camel export --runtime=quarkus --gav=com.foo:acme:1.0-SNAPSHOT --quarkus-group-id=com.redhat.quarkus.platform --quarkus-version=3.20.4.SP1-redhat-00001 --dep=org.apache.camel.quarkus:camel-quarkus-timer,org.apache.camel.quarkus:camel-quarkus-management,org.apache.camel.quarkus:camel-quarkus-cli-connector --directory=camel-quarkus-jbang
```
...
```
camel export --runtime=quarkus --gav=com.foo:acme:1.0-SNAPSHOT --directory=../myproject --quarkus-version=3.20.4.SP1-redhat-00001
```
...
#### [5.8.2. Exporting with Camel CLI included Copy link](#exporting_with_camel_cli_included)
...
#### [5.8.3. Configuring the export Copy link](#configuring_the_export)
...
#### [5.8.4. Configuration Copy link](#configuration)
...
##### [5.8.4.1. Set and unset configuration Copy link](#set_and_unset_configuration)
...
camel config set camel-quarkus-version=3.20
```
...
##### [5.8.4.2. List and get configurations Copy link](#list_and_get_configurations)
...
##### [5.8.4.3. Placeholders substitutes Copy link](#placeholders_substitutes)
...
The placeholder substitution only works for every option that a given Camel command has. You can see all options a command has using `camel run --help` .

## [Chapter 6. Using Camel CLI with Camel Spring Boot Copy link](#camel-cli-sb)

### [6.1. Installing Camel CLI Copy link](#installing-camel-jbang-sb)
...
### [6.2. Using Camel CLI Copy link](#using-camel-jbang-sb)
...
#### [6.2.1. Enable shell completion Copy link](#enable_shell_completion_2)
...
### [6.3. Upgrading Red Hat build of Apache Camel on Spring Boot version with Camel update recipes Copy link](#csb-camel-upgrade-recipes)
...
#### [6.3.1. Running Camel Updates Copy link](#running_camel_updates)

To perform the update to this version, use:

```
$ camel update run 4.10 .3.redhat-00022 --runtime = spring-boot
```
...
##### [6.3.1.1. Configuration Options Copy link](#configuration_options)
...
##### [6.3.1.2. Updating a plain Camel application Copy link](#updating_a_plain_camel_application)

Following example shows how to update a plain Camel application.

```
$ camel update run 4.10 .3.redhat-00022 --runtime = camel-main --repos = https://myMaven/repo --extraActiveRecipes = my.first.Recipe,my.second.Recipe --extraRecipeArtifactCoordinates = ex.my.org:recipes:1.0.0
```
...
##### [6.3.1.3. Updating a Spring Boot application Copy link](#updating_a_spring_boot_application)
...
```
$ camel update run 4.10 .3.redhat-00022 --runtime = spring-boot --extraActiveRecipes = org.openrewrite.java.spring.boot3.UpgradeSpringBoot_3_3 --extraRecipeArtifactCoordinates = org.openrewrite.recipe:rewrite-spring:6.0.2
```
...
### [6.4. Creating and running Camel routes Copy link](#camel-jbang-running-camel-routes-sb)
...
#### [6.4.1. Running routes from multiple files Copy link](#running_routes_from_multiple_files_2)
...
#### [6.4.2. Running routes from input parameter Copy link](#running_routes_from_input_parameter_2)
...
#### [6.4.3. Dev mode with live reload Copy link](#dev_mode_with_live_reload_2)
...
#### [6.4.4. Developer Console Copy link](#developer_console_2)
...
#### [6.4.5. Using profiles Copy link](#using_profiles_2)
...
In the `properties` files you can configure all configurations from [Camel Main](https://camel.apache.org/components/4.10.3/others/main.html) . To turn off and enable log masking run the following command:
...
#### [6.4.6. Downloading JARs over the internet Copy link](#downloading_jars_over_the_internet_2)
...
#### [6.4.7. Adding custom JARs Copy link](#adding_custom_jars_2)
...
#### [6.4.8. Using 3rd-party Maven repositories Copy link](#using_3rd_party_maven_repositories_2)
...
#### [6.4.9. Configuration of Maven usage Copy link](#configuration_of_maven_usage_2)
...
#### [6.4.10. Running routes hosted on GitHub Copy link](#running_routes_hosted_on_github_2)
...
##### [6.4.10.1. Running routes from the GitHub gists Copy link](#running_routes_from_the_github_gists_2)
...
#### [6.4.11. Downloading routes hosted on the GitHub Copy link](#downloading_routes_hosted_on_the_github_2)
...
##### [6.4.11.1. Downloading routes form GitHub gists Copy link](#downloading_routes_form_github_gists_2)
...
#### [6.4.12. Running the Camel K integrations or bindings Copy link](#running_the_camel_k_integrations_or_bindings_2)
...
#### [6.4.13. Run from the clipboard Copy link](#run_from_the_clipboard_2)
...
#### [6.4.14. Controlling the local Camel integrations Copy link](#controlling_the_local_camel_integrations_2)
...
#### [6.4.15. Controlling the Spring Boot and Quarkus integrations Copy link](#controlling_the_spring_boot_and_quarkus_integrations)
...
#### [6.4.16. Getting the status of Camel integrations Copy link](#getting_the_status_of_camel_integrations_2)
...
camel get
  PID   NAME      CAMEL            PLATFORM            READY  STATUS    AGE    TOTAL  FAILED  INFLIGHT  SINCE-LAST
 61818  MyCamel   4.10.3-SNAPSHOT  Spring Boot v2.7.3   1/1   Running  28m34s    854       0         0     0s/0s/-
 63051  test1     4.10.3-SNAPSHOT  JBang                1/1   Running     18s     14       0         0     0s/0s/-
 63068  mygroovy  4.10.3-SNAPSHOT  JBang                1/1   Running      5s      2       0         0     0s/0s/-
```
...
##### [6.4.16.1. Top status of the Camel integrations Copy link](#top_status_of_the_camel_integrations_2)
...
camel top
  PID   NAME     JAVA     CAMEL            PLATFORM            STATUS    AGE         HEAP        NON-HEAP     GC     THREADS   CLASSES
 22104  chuck    11.0.13  4.10.3-SNAPSHOT  JBang               Running   2m10s  131/322/4294 MB  70/73 MB  17ms (6)      7/8  7456/7456
 14242  MyCamel  11.0.13  4.10.3-SNAPSHOT  Spring Boot v2.7.3  Running  33m40s  115/332/4294 MB  62/66 MB  37ms (6)    16/16  8428/8428
 22116  bar      11.0.13  4.10.3-SNAPSHOT  JBang               Running    2m7s   33/268/4294 MB  54/58 MB  20ms (4)      7/8  6104/6104
```
...
##### [6.4.16.2. Starting and Stopping the routes Copy link](#starting_and_stopping_the_routes_2)
...
##### [6.4.16.3. Configuring the logging levels Copy link](#configuring_the_logging_levels_2)
...
##### [6.4.16.4. Listing services Copy link](#listing_services_2)
...
##### [6.4.16.4.1. Listing state of Circuit Breakers Copy link](#listing_state_of_circuit_breakers_2)

If your Camel integration uses the [Circuit Breaker](https://camel.apache.org/components/4.10.x/eips/circuitBreaker-eip.html) , then you can output the status of the breakers with Camel CLI as follows:
...
#### [6.4.17. Scripting from the terminal using pipes Copy link](#scripting_from_the_terminal_using_pipes_2)
...
##### [6.4.17.1. Using stream:in with line vs raw mode Copy link](#using_streamin_with_line_vs_raw_mode_2)

When using `stream:in` to read data from `System in` then the [Stream Component](https://camel.apache.org/components/4.10.x/stream-component.html) works in two modes:
...
#### [6.4.18. Running local Kamelets Copy link](#running_local_kamelets_2)
...
#### [6.4.19. Using the platform-http component Copy link](#using_the_platform_http_component_2)
...
#### [6.4.20. Using Java beans and processors Copy link](#using_java_beans_and_processors_2)
...
#### [6.4.21. Dependency Injection in Java classes Copy link](#dependency_injection_in_java_classes)
...
##### [6.4.21.1. Using Spring Boot dependency injection Copy link](#using_spring_boot_dependency_injection)
...
#### [6.4.22. Debugging Copy link](#debugging_2)
...
##### [6.4.22.1. Java debugging Copy link](#java_debugging_2)
...
##### [6.4.22.2. Camel route debugging Copy link](#camel_route_debugging_2)

The Camel route debugger is available by default (the `camel-debug` component is automatically added to the classpath). By default, it can be reached through JMX at the URL `service:jmx:rmi:///jndi/rmi://localhost:1099/jmxrmi/camel` . You can then use the Integrated Development Environment (IDE) of your choice.

#### [6.4.23. Health Checks Copy link](#health_checks_2)
...
### [6.5. Listing what Camel components is available Copy link](#camel-jbang-listing-camel-components-sb)
...
#### [6.5.1. Displaying component documentation Copy link](#displaying_component_documentation_2)
...
##### [6.5.1.1. Browsing online documentation from the Camel website Copy link](#browsing_online_documentation_from_the_camel_website_2)
...
##### [6.5.1.2. Filtering options listed in the tables Copy link](#filtering_options_listed_in_the_tables_2)
...
### [6.6. Gathering list of dependencies Copy link](#camel-jbang-list-of-dependencies-sb)
...
org.apache.camel.springboot:camel-spring-boot-engine-starter:4.10.0
org.apache.camel.springboot:camel-dsl-modeline-starter:4.10.0
org.apache.camel.springboot:camel-kamelet-starter:4.10.0
org.apache.camel.springboot:camel-log-starter:4.10.0
org.apache.camel.springboot:camel-rest-starter:4.10.0
org.apache.camel.springboot:camel-stream-starter:4.10.0
org.apache.camel.springboot:camel-timer-starter:4.10.0
...
### [6.7. Open API Copy link](#camel-jbang-open-api-sb)

Camel CLI allows to quickly expose an Open API service using `contract first` approach, where you have an existing OpenAPI specification file. Camel CLI bridges each API endpoints from the OpenAPI specification to a Camel route with the naming convention `direct:<operationId>` . This make it quicker to implement a Camel route for a given operation.

See the [OpenAPI example](https://github.com/apache/camel-kamelets-examples/tree/main/jbang/open-api) for more details.

### [6.8. Troubleshooting Copy link](#camel-jbang-troubleshooting-sb)

When you use JBang, it stores the state in `~/.jbang` directory. This is also the location where JBang stores downloaded JARs. Camel CLI also downloads the needed dependencies while running.

However, these dependencies are downloaded to your local Maven repository `~/.m2` . So when you troubleshoot the problems such as an outdated JAR while running the Camel CLI, try to delete these directories, or parts of it.

### [6.9. Exporting to Camel Spring Boot Copy link](#camel-jbang-exporting-to-camel-spring-boot-sb)
...
#### [6.9.1. Exporting to Camel Spring Boot Copy link](#exporting_to_camel_spring_boot)
...
#### [6.9.2. Exporting with Camel CLI included Copy link](#exporting_with_camel_cli_included_2)
...
#### [6.9.3. Configuring the export Copy link](#configuring_the_export_2)
...
#### [6.9.4. Configuration Copy link](#configuration_2)
...
##### [6.9.4.1. Set and unset configuration Copy link](#set_and_unset_configuration_2)
...
camel config set build-property=openshift-maven-plugin-version=1.16.1.redhat-00018
...
##### [6.9.4.2. List and get configurations Copy link](#list_and_get_configurations_2)
...
##### [6.9.4.3. Placeholders substitutes Copy link](#placeholders_substitutes_2)
...
#### [6.9.5. Troubleshooting Copy link](#troubleshooting)
...
## [Legal Notice Copy link](#idm140462613985360)

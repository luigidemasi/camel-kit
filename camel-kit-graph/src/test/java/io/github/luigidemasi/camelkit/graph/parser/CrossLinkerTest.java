...
class CrossLinkerTest {
    ...
    void linksDirectEndpoints() {
        graph.addNode(new GraphNode(
                "route:routeA", NodeType.CAMEL_ROUTE, Map.of("routeId", "routeA", "fromUri", "kafka:input")));
        graph.addNode(new GraphNode(
                "route:routeB", NodeType.CAMEL_ROUTE, Map.of("routeId", "routeB", "fromUri", "direct:foo")));
        graph.addNode(new GraphNode(
                "endpoint:direct:foo", NodeType.CAMEL_ENDPOINT, Map.of("uri", "direct:foo", "scheme", "direct")));
        graph.addNode(new GraphNode(
                "endpoint:kafka:input", NodeType.CAMEL_ENDPOINT, Map.of("uri", "kafka:input", "scheme", "kafka")));
    ...
    void linksSedaEndpoints() {
        graph.addNode(new GraphNode(
                "route:routeA", NodeType.CAMEL_ROUTE, Map.of("routeId", "routeA", "fromUri", "timer:tick")));
        graph.addNode(new GraphNode(
                "route:routeB", NodeType.CAMEL_ROUTE, Map.of("routeId", "routeB", "fromUri", "seda:queue")));
        graph.addNode(new GraphNode(
                "endpoint:seda:queue", NodeType.CAMEL_ENDPOINT, Map.of("uri", "seda:queue", "scheme", "seda")));
    ...
    void linksComponentToMavenArtifact() {
        graph.addNode(
                new GraphNode("route:r1", NodeType.CAMEL_ROUTE, Map.of("routeId", "r1", "fromUri", "kafka:topic")));
        graph.addNode(new GraphNode(
                "endpoint:kafka:topic", NodeType.CAMEL_ENDPOINT, Map.of("uri", "kafka:topic", "scheme", "kafka")));
        graph.addNode(new GraphNode(
                "maven:org.apache.camel:camel-kafka", NodeType.MAVEN_ARTIFACT, Map.of("artifactId", "camel-kafka")));
        graph.addEdge(new GraphEdge("route:r1", "endpoint:kafka:topic", EdgeType.ROUTES_FROM, Map.of()));
        new CrossLinker().link(graph);
        List<GraphEdge> usesComponent
                = graph.getEdges().stream().filter(e -> e.type() == EdgeType.USES_COMPONENT).toList();

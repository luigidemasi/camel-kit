...
2. [Preface](#idm139881064348752)
3. 1. Migrating from Camel-K
4. [Legal Notice](#idm139881062162096)
...
## Information about how to migrate from Camel K to Red Hat build of Apache Camel for Quarkus.

[Legal Notice](#idm139881062162096)

**Abstract**

This guide provides information about how to migrate from Camel K to Red Hat build of Apache Camel for Quarkus.

## [Preface Copy link](#idm139881064348752)
...
- The target system to deploy is an OpenShift Cluster {openshift-version}+.
- Camel K version is {camel-k-version}.
...
- Link:https://camel.apache.org/manual/camel-jbang.html#\_installation[Camel JBang {jbang-version}]
- JDK {jdk-version-all}
- Maven (mvn cli) 4.10.6
- [oc cli](https://mirror.openshift.com/pub/openshift-v4/clients/ocp/%7Boc-cli-version%7D/)
- OpenShift cluster {openshift-version}+
...
For more information about properties, see [Quarkus OpenShift Extension](https://quarkus.io/version/%7Bquarkus-cxf-url-version%7D/guides/deploying-to-openshift) .
...
Track the artifact name in the generated pom, as the artifact name is used in the generated Openshift files (Deployment, Service, Route, and so on).
...
Switch to the maven project directory:
...
(Optional) Change the compiler version to 21 in `pom.xml` :
...
Create a local ConfigMap file named `ceq-app in `src/main/kubernetes/common.yml` which will be a part of the image build process.

The following command sets the ConfigMap key as `application.properties` :
...
(Optional) Change the base image to OpenJDK 21 in `src/main/docker` :
...
To set the image address in the generated `openshift.yml` and `knative.yml` file, you must set these container image properties:
...
To allow the Knative controller to inject the `K_SINK` environment variable to the deployment, add the following property in `application.properties`
...
To allow Camel to load the Knative environment configuration, add the following property:
...
To set the image address in the generated openshift.yml and knative.yml file, set these container image properties:
...
This should show something like the following output:
...
#### [1.8.3. Quarkus build fails with invalid certification path Copy link](#quarkus_build_fails_with_invalid_certification_path)
...
- [Quarkus Configuration for Kubernetes](https://quarkus.io/version/%7Bquarkus-cxf-url-version%7D/guides/deploying-to-kubernetes)
- [Quarkus Configuration for Openshift](https://quarkus.io/version/%7Bquarkus-cxf-url-version%7D/guides/deploying-to-openshift)

## [Legal Notice Copy link](#idm139881062162096)
